...
def find_tool(name):
    ...
    # Primeiro procura usando o PATH correto
    path = shutil.which(name, path=env['PATH'])

    if path:
        return path

    # Caminhos conhecidos do devkitPro
    known_paths = {
...
def run_build(project, log_file):
    ...
    if not make_path:
        raise RuntimeError('Não encontrei o comando make no servidor.')
...
<li>
Escolha o ZIP do projeto C.
</li>

<li>
O Builder procura o Makefile.
</li>

<li>
O servidor usa devkitARM/libnds.
</li>

<li>
Se der certo, aparece o .nds.
</li>
...
() => {
    ...
    ? file.files[0].name +
      ' (' +
      Math.round(file.files[0].size / 1024) +
      ' KB)'
    : 'Nenhum arquivo escolhido';
...
def build():
    ...
    if (
        not uploaded
        or
        not uploaded.filename.lower().endswith('.zip')
    ):
...
def download(job_id):
    ...
    artifact =
    BASE / job_id / 'out' / 'ButtonRush.nds'
